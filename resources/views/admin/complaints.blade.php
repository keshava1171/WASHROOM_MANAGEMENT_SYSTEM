...
@section('title', 'Complaints Management | Admin | WMS')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-fade-in-up">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight">Intelligence Forge</h1>
            <p class="text-slate-600 dark:text-slate-400">Incoming feedback and critical issue resolution</p>
        </div>
        
        <div class="flex items-center gap-4">
            @if($complaints->total() > 0)
                <a href="{{ route('admin.complaints.export') }}" class="btn-secondary py-3 px-6 rounded-2xl flex items-center shadow-sm hover:text-emerald-600 transition-colors">
                    <i class="fas fa-file-csv mr-2 text-lg text-emerald-500"></i>
                    <span class="text-xs font-bold uppercase tracking-widest">Export Registry</span>
                </a>
            @endif

            <div class="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 px-6 py-3 rounded-2xl flex items-center pr-12 shadow-sm">
                <div class="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 flex items-center justify-center mr-4">
                    <i class="fas fa-exclamation-circle text-lg animate-pulse"></i>
                </div>
                <div>
                    <div class="text-xs font-bold text-amber-600/70 dark:text-amber-400/70 uppercase tracking-widest mb-0.5">Pending Action</div>
                    <div class="text-2xl font-extrabold text-amber-700 dark:text-amber-400 leading-none">
                        {{ $complaints->total() }}
                    </div>
                </div>
            </div>
    ...
    <div class="space-y-4 animate-fade-in-up" style="animation-delay: 0.1s">
        <div class="premium-card p-6 flex flex-col md:flex-row gap-6 items-center justify-between">
            <form action="{{ route('admin.complaints') }}" method="GET" class="flex flex-wrap md:flex-nowrap gap-4 items-center flex-1 w-full">
                <div class="relative flex-1 min-w-[200px]">
                    <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search ID, Name, or Description..." 
                           class="input-premium pl-11 py-3 text-sm w-full">
                </div>
                
                <select name="status" class="input-premium py-3 px-8 text-sm w-full md:w-48 bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 rounded-xl font-bold">
                ...
                <button type="submit" class="btn-primary py-3 px-8 shadow-lg text-sm whitespace-nowrap font-black uppercase tracking-widest">
                    <i class="fas fa-filter mr-2"></i> Filter
                </button>
                @if(request()->anyFilled(['search', 'status']))
                    <a href="{{ route('admin.complaints') }}" class="w-12 h-12 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-rose-500 transition-colors">
                        <i class="fas fa-times"></i>
                    </a>
                @endif
            </form>

            <div class="flex items-center gap-3 w-full md:w-auto pt-4 md:pt-0 border-t md:border-t-0 border-slate-200 dark:border-slate-800">
            ...
            <div class="premium-card flex flex-col h-full border-t-8 {{ $complaint->status == 'pending' ? 'border-t-amber-500' : ($complaint->status == 'in_progress' ? 'border-t-indigo-500' : 'border-t-emerald-500') }} shadow-2xl bg-white/80 dark:bg-slate-900/60 backdrop-blur-3xl">
                <div class="p-6 flex-1 flex flex-col">
                    ...
                    <div class="flex items-center gap-2 mb-4">
                        <span class="flex items-center bg-slate-100 dark:bg-slate-800 px-3 py-1 text-[10px] font-bold text-slate-500 dark:text-slate-400 rounded border border-slate-200 dark:border-slate-700">
                            <i class="fas fa-user-circle mr-1.5 opacity-60"></i>
                            Reporter: {{ $complaint->user->name ?? 'SYSTEM NODE' }}
                        </span>
                    </div>
                    ...
                    <div class="flex flex-col gap-4 flex-1">
                        ...
                        <div class="space-y-1.5 flex-1">
                            <p class="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-1">Tactical Intel</p>
                ...
                <div class="px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800">
                    <form action="{{ route('admin.complaints.update', $complaint) }}" method="POST" class="space-y-3">
                        @csrf
                        @method('PATCH')
                        <div class="flex flex-col xl:flex-row gap-3">
                            <div class="flex-1 space-y-1">
                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Resolution & Feedback</label>
                                <textarea name="admin_notes" rows="2" placeholder="Action taken or response..." 
                            ...
                            <div class="w-full xl:w-36 space-y-1">
                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Protocol Status</label>
                        ...
                        @if($complaint->status !== 'resolved')
                            <div class="flex items-center justify-between gap-4">
                                ...
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('complaints.print', $complaint->id) }}" target="_blank" class="px-3 py-2 rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors" title="Print Incident Paper">
                                        <i class="fas fa-print"></i>
                                    </a>
                                    <button type="submit" class="btn-primary py-2 px-4 shadow-sm text-[10px] font-black uppercase tracking-widest">
                                        <i class="fas fa-sync-alt mr-2"></i> Update Protocol
                                    </button>
                                </div>
                            </div>
                        @else
...
<script>
    ...
    async function executeBulkAction(action) {
        ...
        try {
            const response = await fetch("{{ route('admin.complaints.bulk') }}", {
                method: 'POST',
                headers: {
